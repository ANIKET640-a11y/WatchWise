...
class TMDBMovieDetails(BaseModel):
    ...
    videos: List[dict] = []   # all YouTube trailer/teaser keys for fallback chain
    reviews: List[dict] = []
    cast: List[dict] = []
...
async def tmdb_movie_details(movie_id: int) -> TMDBMovieDetails:
    ...
    trailer_url, all_videos = await get_movie_trailer(movie_id)
...
async def get_movie_trailer(movie_id: int):
    """Returns (primary_url, all_video_dicts) where all_video_dicts is a
    list of {key, type, name} for every YouTube trailer/teaser — used as
    a fallback chain on the frontend when embed is disabled (Error 153)."""
    try:
        data = await tmdb_get(f"/movie/{movie_id}/videos", {"language": "en-US"})
        results = data.get("results", [])

        # Prefer official trailers, then teasers, then clips
        priority = {"Trailer": 0, "Teaser": 1, "Clip": 2, "Featurette": 3}
        yt_videos = [
            v for v in results
            if v.get("site") == "YouTube" and v.get("key")
        ]
        yt_videos.sort(key=lambda v: (
            priority.get(v.get("type", ""), 9),
        ...
        all_videos = []
        for v in yt_videos:
            if primary_url is None and v.get("type") == "Trailer":
                primary_url = f"https://www.youtube.com/watch?v={v['key']}"
            all_videos.append({"key": v["key"], "type": v.get("type",""), "name": v.get("name","")})

        # If no Trailer found, use first available video as primary
        if primary_url is None and yt_videos:
            primary_url = f"https://www.youtube.com/watch?v={yt_videos[0]['key']}"

        return primary_url, all_videos
    except Exception:
        return None, []
...
async def get_movie_cast(movie_id: int, limit: int = 15):
    try:
        data = await tmdb_get(
        ...
        for person in data.get("cast", [])[:limit]:
            cast.append({
                "id": person.get("id"),          # TMDB person ID — needed for celeb detail page
...
# ---------- DISCOVER BY GENRE (for genre browse page) ----------
@app.get("/discover/genre", response_model=List[TMDBMovieCard])
async def discover_by_genre(
    ...
    """
    Discover movies by genre_id with optional filters.
    Used by the Genre Browse page in the frontend.
    sort_by options: popularity.desc, vote_average.desc, release_date.desc, release_date.asc
    """
    valid_sorts = {"popularity.desc", "vote_average.desc", "release_date.desc", "release_date.asc", "primary_release_date.desc"}
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Genre discover failed: {e}")
